...
const { charRange, xor } = require("../../utils");
...
function isValidChar(char) {
    ...
    return (
        charCode == 10 || // New line
        charCode == 13 || // Carriage return
        charCode == 32 || // White-space
        charCode == 33 || // Exclamation mark
        charCode == 34 || // Double quote mark
        charCode == 39 || // Single quote mark
        charCode == 40 || // Opening pharenthesis
        charCode == 41 || // Closing pharenthesis
        charCode == 44 || // Comma
        charCode == 45 || // Dash
        charCode == 46 || // Period
        (charCode > 47 && charCode < 58) || // Integer numbers (0-9)
        charCode == 58 || // Colon
        charCode == 59 || // Semicolon
        charCode == 63 || // Question mark
        (charCode > 64 && charCode < 91) || // Uppercase letters (A-Z)
        charCode == 91 || // Opening bracket
        charCode == 93 || // Closing bracket
...
function findPlaintext(ciphertext) {
    ...
    return candidateKeys.reduce(
        (previousFind, candidateKey) => {
            const candidateKeyBuffer = Buffer.allocUnsafe(
                ciphertextBuffer.length
            ).fill(candidateKey);
            const candidatePlaintextBuffer = xor(
                ciphertextBuffer,
                candidateKeyBuffer
            );
